...
public class AnnonceAvecPhoto {
    ...
    public void getListePhoto (Connection connection)throws Exception{
        ...
        try(PreparedStatement preparedStatement = connection.prepareStatement(sql)) {
            ...
            System.out.println(sql+" idAnnonce"+this.getIdAnnonce());
            ...
            try (ResultSet resultSet = preparedStatement.executeQuery()) {
                while (resultSet.next()) {
                    ...
                    System.out.println(nomPhotoVoiture);
...
public List<AnnonceAvecPhoto> listeAnnonceWithPhoto2 (Connection connection)throws Exception{
    String sql= "SELECT * FROM annonceDetaille where etatannonce!=1";
    List<AnnonceAvecPhoto> listes = new ArrayList<>();
    ...
    try(PreparedStatement preparedStatement = connection.prepareStatement(sql)) {
        ...
        try (ResultSet resultSet = preparedStatement.executeQuery()) {
            while (resultSet.next()) {
                ...
                String nomUtilisteur = resultSet.getString("nomutilisateur");
                String nomLieu = resultSet.getString("nomLieu");
                int annee = resultSet.getInt("annee") ;
                double prix = resultSet.getDouble("prix");
                int pourcentage = resultSet.getInt("pourcentage") ;
                ...
                annonceWithPictures.setNomCategorie(nomCategorie);
                annonceWithPictures.setNomMarque(nomMarque);
                annonceWithPictures.setNomCarburant(nomCarburant);
                annonceWithPictures.setNomModel(nomModel);
                annonceWithPictures.setPourcentage(pourcentage);
                annonceWithPictures.setNomBoiteDeVitesse(nomBoiteDeVitesse);
                annonceWithPictures.setIdCreateur(idcreateur);
                annonceWithPictures.getListePhoto(connection);
...
public List<AnnonceAvecPhoto> historiqueParUtilisateur (Connection connection , int idCreateur)throws Exception{
    String sql= "SELECT * FROM annonceDetaille where idcreateur="+idCreateur+" order by etatannonce asc";
